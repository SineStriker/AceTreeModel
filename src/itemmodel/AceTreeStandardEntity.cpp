...
AceTreeStandardEntityPrivate::AceTreeStandardEntityPrivate(AceTreeStandardEntity::Type type)
    : type(type) {
    m_external = false;
}
...
AceTreeEntityVectorPrivate::AceTreeEntityVectorPrivate()
    : AceTreeStandardEntityPrivate(AceTreeStandardEntity::Vector) {
    _signals = nullptr;
}
...
void AceTreeEntityMappingPrivate::event(AceTreeEvent *event) {
    ...
    switch (event->type()) {
        case AceTreeEvent::DynamicDataChange: {
            ...
            auto it = dynamicPropertyNotifiers.find(e->key());
            if (it == dynamicPropertyNotifiers.end())
                return;
            it.value()(q, e->value(), e->oldValue());
            break;
        }

        case AceTreeEvent::PropertyChange: {
            auto e = static_cast<AceTreeValueEvent *>(event);
            auto it = propertyNotifiers.find(e->key());
