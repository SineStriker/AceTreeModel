...
bool AceTreeItem::addSubscriber(AceTreeItemSubscriber *sub) {
    Q_D(AceTreeItem);
    if (d->subscribers_indexes.contains(sub))
        return false;

    // Add to list
    auto it = d->subscribers.insert(d->subscribers.end(), sub);

    // Add to hash
    d->subscribers_indexes.insert(sub, it);
...
bool AceTreeItem::removeSubscriber(AceTreeItemSubscriber *sub) {
    ...
    auto it = d->subscribers_indexes.find(sub);
    if (it == d->subscribers_indexes.end())
        return false;
    ...
    // Remove from list
    d->subscribers.erase(it.value());

    // Remove from hash
    d->subscribers_indexes.erase(it);
...
QList<AceTreeItemSubscriber *> AceTreeItem::subscribers() const {
    ...
    return QList<AceTreeItemSubscriber *>(d->subscribers.begin(), d->subscribers.end());
}
...
bool AceTreeItem::hasSubscriber(AceTreeItemSubscriber *sub) const {
    Q_D(const AceTreeItem);
    return d->subscribers_indexes.contains(sub);
}
